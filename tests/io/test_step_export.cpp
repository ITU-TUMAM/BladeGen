...
{
    return fs::temp_directory_path() / "bladegen_test_export.step";
}
...
{
    const auto sphere = BladeGen::Geometry::MakeSphere(3.0);
    const auto box    = BladeGen::Geometry::MakeBox(4.0, 4.0, 4.0);
...
{
    const auto shape = BladeGen::Geometry::MakeBox(2.0, 2.0, 2.0);
    REQUIRE(shape.has_value());

    const auto path = TempStepPath();

    BladeGen::IO::StepExporter exp(BladeGen::IO::StepProtocol::AP203);
